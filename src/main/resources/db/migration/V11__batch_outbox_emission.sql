-- Lets the batch writer opt in to the outbox, so the hybrid's central assumption can be
-- measured on BOTH sides.
--
-- Fault F11 showed the hybrid losing 69 % of events from a PL/SQL batch — identically to the
-- plain wrapper. That is the correct result for an UNMODIFIED legacy writer, which is the case
-- the original generator models. But the hybrid's claim is not that legacy writers are captured
-- automatically; it is that they are captured once they write through the shared EMIT_A1_EVENT
-- contract. Measuring only the unmodified case tests half the claim and reports it as the whole.
--
-- p_emit_outbox = TRUE makes the batch honour the contract. Comparing the two runs turns the
-- assumption from a caveat in Chapter 7 into a measured pair.
--
-- Also fixes fault F5, which held a transaction open past the connector's retention using this
-- generator. Without outbox rows every outbox-based mode reported the events missing for the
-- F11 reason, so the retention path was never exercised at all.
--
-- This file is V7 reproduced in full with three changes: p_emit_outbox on the signature (spec
-- and body), a nested emit() helper, and four call sites. Reproducing the whole package is the
-- only way Oracle allows one procedure inside it to be replaced.
...
CREATE OR REPLACE PACKAGE PKG_SYNTH_DATA AS
  ...
  -- p_emit_outbox makes the batch write through EMIT_A1_EVENT, modelling a legacy writer that
  -- HAS been modified to honour the outbox contract. Default FALSE keeps the original
  -- behaviour, which models one that has not — the two halves of fault case F11.
  PROCEDURE generate_transactions(p_count        IN NUMBER,
                                  ...
                                  p_emit_outbox  IN BOOLEAN  DEFAULT FALSE);
...
CREATE OR REPLACE PACKAGE BODY PKG_SYNTH_DATA AS
  ...
  PROCEDURE generate_transactions(p_count        IN NUMBER,
                                  ...
                                  p_advance      IN BOOLEAN  DEFAULT TRUE,
                                  p_commit_every IN NUMBER   DEFAULT 5000,
                                  p_emit_outbox  IN BOOLEAN  DEFAULT FALSE) IS
    l_currency VARCHAR2(3);
    l_segment  VARCHAR2(12);
    l_debit    VARCHAR2(34);
    ...
    -- Writes one event through the SAME contract the application uses, so a batch that opts in
    -- is indistinguishable downstream from an application write. Payload construction is
    -- delegated to PKG_A1_EVENT so the batch cannot drift into a different event shape than
    -- the trigger path produces.
    PROCEDURE emit(p_txn_id IN NUMBER, p_old IN VARCHAR2, p_new IN VARCHAR2) IS
      l_type     VARCHAR2(64);
      l_event_id VARCHAR2(32);
    BEGIN
      IF NOT p_emit_outbox THEN RETURN; END IF;
      l_type := PKG_A1_EVENT.event_type_for(p_old, p_new);
      IF l_type IS NULL THEN RETURN; END IF;
      l_event_id := RAWTOHEX(SYS_GUID());
      EMIT_A1_EVENT(TO_CHAR(p_txn_id), l_type, p_source,
                    PKG_A1_EVENT.build_payload(l_event_id, p_txn_id, l_type,
                                               p_old, p_new, p_source));
    END emit;
  BEGIN
    load_pool;
